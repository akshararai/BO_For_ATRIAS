function [nm, nmc, parOffR] = fn_set_vnmc_CtrlParams(nm, param)
...
parRange = fn_set_ParRange();

v_parOffR	= zeros(length(param),1);
for iP = 1:length(param)
    % calculate how off param is
    v_parOffR(iP) = v_parOffR(iP) + (-param(iP)+parRange(iP,1))*(param(iP)<parRange(iP,1));
    v_parOffR(iP) = v_parOffR(iP) + (param(iP)-parRange(iP,2))*(param(iP)>parRange(iP,2));
    
    % keep param in range
    param(iP)	= max(parRange(iP,1), min(parRange(iP,2), param(iP)));
end
parOffR = sum(v_parOffR);
...
St_FGain_VAS     	= parStCtrl(7)      *1.1/nm.FmaxVAS;

% M2: prevent knee overextension
St_phiK0            = (180 - 5*parStCtrl(8))*pi/180;
St_phiKGain_VAS     = parStCtrl(9)      *10;
St_phiKGain_BFSH	= parStCtrl(10)     *5;

% M3: balance trunk
St_GainTh_HFL       = parStCtrl(11)     *1;
St_GainDTh_HFL      = parStCtrl(12)     *0.3;
St_GainTh_GLU       = parStCtrl(13)     *1;
St_GainDTh_GLU      = parStCtrl(14)     *0.3;
St_SGainGLU_HAM     = parStCtrl(15)     *1;

% M4: compensate swing leg
St_SGainCGLU_HFL    = parStCtrl(16)     *.1;
St_SGainCHAM_HFL    = parStCtrl(17)     *.1;
St_SGainCHFL_GLU    = parStCtrl(18)     *.1;
St_SGainCRF_GLU     = parStCtrl(19)     *.1;
...
St_M1       = [St_FGain_VAS];
St_M2       = [St_phiK0 St_phiKGain_VAS St_phiKGain_BFSH];
...
% swing Ctrl (knee_i)
Sw_dalpGain_BFSH   	= parSwCtrl(13)     *1.0;

% swing Ctrl (knee_ii)
Sw_dphiKGain_RF  	= parSwCtrl(14)     *.1;
Sw_SRF_thr          = parSwCtrl(15)     *.1;
...
Sw_phiK0            = (180 - 5*parSwCtrl(23))*pi/180;
...
%%% nm.params
nmc.BLCtrlPar       = CtrlPar_theta0;
nmc.FPCtrlPar   	= FPCtrlPar;
nmc.LLCtrlPar    	= LLCtrlPar;
nmc.StCtrlPar   	= StCtrlPar;
nmc.SwCtrlPar    	= SwCtrlPar;
nmc.TrCtrlPar     	= TrCtrlPar;

nm.St_i             = St_i;
nm.parSt_PreS_i     = parSt_PreS_i;
nm.parSt_M1_i       = parSt_M1_i;
nm.parSt_M2_i       = parSt_M2_i;
nm.parSt_M3_i       = parSt_M3_i;
nm.parSt_M4_i       = parSt_M4_i;

nm.Sw_i             = Sw_i;
nm.parSwPreS_i      = parSwPreS_i;
nm.parSwPh_i        = parSwPh_i;
nm.parSwH_i         = parSwH_i;
nm.parSwKi_i        = parSwKi_i;
nm.parSwKii_i       = parSwKii_i;
nm.parSwKiii_i      = parSwKiii_i;
nm.parSwKiv_i       = parSwKiv_i;