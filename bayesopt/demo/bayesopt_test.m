...
%bayesoptF = @bayesopt_orig
bayesoptF = @bayesoptBARE
...
[min_sample, min_value, botrace] = bayesoptF(str2func('braninf'), opt);

ttime = toc(tstart);
fprintf('BO braninf: f_min %f, x* (%f %f) time %f sec\n', min_value, min_sample, ttime);
...
opt.maxes = [3, 2];
tstart = tic;
[min_sample, min_value, botrace] = bayesoptF(str2func('camelf'), opt);
ttime = toc(tstart);
fprintf('BO camelf: f_min %f, x* (%f %f) time %f sec\n', ...
    min_value, min_sample, ttime);
assert(min_value+1.0316 < 0.01)
...
%[min_sample, min_value, botrace] = bayesoptF(F, opt);
